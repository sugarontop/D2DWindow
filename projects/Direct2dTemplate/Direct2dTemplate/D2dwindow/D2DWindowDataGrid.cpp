...
void D2DGridControl::OnCreate()
{
	...
	color_mouse_move_ = MakeBrsuh( parent_->cxt_, COLOR_DEF_FLOAT_SELECT);
	color_selected_ = MakeBrsuh( parent_->cxt_, COLOR_DEF_SELECTED);
}

// データの加算、削除などで行数が変化する場合に実行
void D2DGridControl::DataUpdate()
{
	data_cnt_ = serial_data_->size();
	float h = data_cnt_ * item_min_height_;
	bar_->SetTotalSize( h );

	selected_idx_ = min( selected_idx_, data_cnt_-1);
	float_idx_ = -1;
	...
	for( UINT i = 0; i < (UINT)data_cnt_; i++ )
	{
		ItemsHeight_[i] = item_min_height_;
	}
		...
		mat.PushTransform();
		{
			...
			{		
				...
				if ( data_idx < (int)data_cnt_ )
				{
					...
					bd.data = (void*)(serial_data_)->at(data_idx);
					bd.color_mouse_move_back = color_mouse_move_;
					bd.color_selected_back = color_selected_;
					bd.color_selected_text = cxt.black;


					t->Bind( parent_, bd );// (void*)(serial_data_)->at(data_idx)); //serial_constent_data_[data_idx]) );
...
void D2DStatsuControlItem::Draw( D2DContext& cxt, FSizeF& sz, int stat, void* etc )
{
	...
	StatusSequentialData::Row* s = (StatusSequentialData::Row*)data_.data;

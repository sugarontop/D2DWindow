// DDDTestJS.cpp : アプリケーションのエントリ ポイントを定義します。
//
	...
	case WM_CREATE:
	{
		auto DDDControlCreate = [hWnd](D2DWin w)
		{
			...
			DDEvent0(D2EVENT0_MODE::CLICK, btnLoad, [hWnd,txm](D2EVENT0_MODE ev, D2Ctrl)
			{
				...
				BSTR src;
				if ( DDOpenTextFile( L"test.js", &src))
				{
		...
		auto MessageFromClient = [hWnd]( LPCWSTR msg ){

			BSTR sss = ::SysAllocString( msg );

			::SendMessage(hWnd, WM_D2D_FREE, 0, (LPARAM)sss);

		};

		SRV_StartServer(MessageFromClient);

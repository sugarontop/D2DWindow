...
//---------------------------------------------------------------------------------------------------------------
// most important function. 行数の把握と文字単位にPOS,LEN,RECTなどの情報を取得
//
//---------------------------------------------------------------------------------------------------------------
...
BOOL CTextLayout::Layout(D2DContext& cxt, const WCHAR *psz, int nCnt,const SIZE& sz, bool bSingleLine, int zCaret, int& StarCharPos,IDWriteTextFormat* fmt)
{
	...
	{		
		...
		{		
			...
			{
				singleline_row_width_ += prgLines_[0].prgCharInfo[col].rc.Width();
			}
		}
...
{
	{
		...
		cxt.cxt->DrawTextLayout( FPointF(), DWTextLayout_, cxt.forecolor_ );
	}

    // 以下 Render selection area and caret.
        ...
        {           
			...
			if ((nSelEnd >= prgLines_[r].nPos) && (nSelStart <= prgLines_[r].nPos + prgLines_[r].nCnt))			
            {
				...
				if (nSelStartInLine != nSelEndInLine && nSelEndInLine != -1)
                {
                    ...
                    {
                        InvertRect(cxt, &prgLines_[r].prgCharInfo[j].rc);  //halfトーン色でfill
                    }

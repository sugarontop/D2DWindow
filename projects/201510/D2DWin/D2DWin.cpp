// D2DWin.cpp : DLL アプリケーション用にエクスポートされる関数を定義します。
//
...
DLLEXPORT D2Ctrl WINAPI DDMkImageButtons(D2Ctrls ctrls, const FRectFBM& rc, LPCWSTR name, DDImage btnImage, const D2D1_RECT_F* btnrc, int btnrc_cnt)
{
	auto parent = (D2DControls*) ctrls.ctrls;
	...
	{
		mds[i] = 0;
		mds[i] = MODE_ENABLE| MODE_ONOFF;
	}

	btns->SetButtons( false, btnrc, mds, btnrc_cnt);
	btns->SetBackColor( D2RGBA(242, 242, 242,0), D2RGBA(26, 123, 253,200), D2RGBA(153, 217, 234,150), D2RGB(150, 150, 150));
...
DLLEXPORT D2Ctrls WINAPI DDMkVerticalStackControls(D2Ctrls ctrls, const FRectFBM& rc1, LPCWSTR classnm, LPCWSTR name)
{
	auto parent = (D2DControls*) ctrls.ctrls;
	FRectFBM rcf = rc1;
	if (rcf.right - rcf.left <= 0)
	{
		rcf.SetRect( FPointF(0,0), parent->GetContentRect().Size());		
	}
